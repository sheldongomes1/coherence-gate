...
## ADR-34: ADK packages the gate for Agent Engine; it does not orchestrate it

**Date:** 2026-09-14
**Status:** Accepted

**Context:** Phase 2 always named an ADK wrapper, and the obvious reading of "use an agent framework" is to let the framework run the pipeline. But the pipeline is a fixed DAG whose order is a correctness property: parse, two independent readings, normalize and merge, compare under the tolerance table, lanes, then explain. A framework that lets a model choose the next step would end the sentence the product rests on. Separately, the deployment story (Agent Engine, Cloud Trace, one Google governance surface) is real and is what the bank's architects will ask about.

**Decision:** `src/coherence_gate/adk/` wraps the gate as a `SequentialAgent` over three stages — `read_document`, a `ParallelAgent` running the two family extractors, then `decide_and_explain` — using ADK's deterministic workflow agents only. No `LlmAgent` appears anywhere in the tree, and a test asserts that. Each agent calls the SAME function the direct path calls; the read stage was factored out of `run_document` into `pipeline.read_document` so both paths share it rather than each having its own. Every stage emits an ADK event carrying the facts a reader wants (sha, product type, fields cited versus declared absent versus malformed, lane, findings, attested hash). `cg run --via adk` runs one document through it.

**Alternatives considered:** (a) Let ADK orchestrate with an LLM planner: rejected, it moves the control flow into a model. (b) Re-express each deterministic step as its own agent: rejected, it creates a second implementation of the decision, and two implementations eventually disagree; the acceptance test would then be comparing the system to itself. (c) Skip ADK and keep the loop: that is still what the eval and the service do; the wrapper is additive, for the deployment path. (d) Make ADK a normal dependency: rejected for a measured reason below.

**Consequences:** The acceptance test is the artifact: the same document through both paths produces identical findings, identical lanes and identical attestation hashes, so the framework demonstrably changes nothing about the result. ADK stays an OPTIONAL install (`uv pip install google-adk`, `make adk-check`) and is deliberately absent from `pyproject.toml` and `uv.lock`, because adding it forced uv's universal resolution to downgrade two transitive pins the demo image depends on (`opentelemetry-api` 1.44.0 → 1.42.1, `websockets` 16.1.1 → 15.0.1). A framework that quietly moves the service's dependency floor is a framework that belongs in its own environment, which is exactly what Agent Engine gives it. The eval, the CLI and the live service import nothing from this package, and the demo image's requirement set is byte-identical with the wrapper in the tree.

## ADR-35: The trace goes to BigQuery through the `bq` CLI, and the questions become views

**Date:** 2026-09-14
**Status:** Accepted

**Context:** `trace.jsonl` answers "what did this run do" and the provenance graph (ADR-33) draws one document from it, but the questions that matter in production only exist *across* runs: does a family's disagreement rate on a field move after a pin change, what share of calls never completed this month, what does a document cost by product. Those are the drift signals, and a file per run cannot answer them. Phase 2 named a BigQuery sink; release day gave it three real runs worth loading, including two failures.

**Decision:** `cg trace-export --run <dir>` converts a run's trace into newline-delimited JSON plus a schema and loads it with the `bq` CLI into a table partitioned by day and clustered by `run_id, step, doc_id`. Two columns are derived rather than stored raw: `family` (from the step name) and `completed` (false for `API_ERROR`, `TIMEOUT`, `REFUSAL`, `DEADLINE`), because "the model never answered" is the distinction a cost or accuracy number silently hides. Three views ship with it: `v_cost_by_run` (readings versus desk queries, with the reused count so a resumed run is not read as a cheap one), `v_never_completed` (billing and transport events by run and family), `v_family_profile` (provider calls, reused calls, p50/p95 latency, thinking volume, cost). Loading is idempotent: a run already present is skipped unless `--replace`, because loading twice doubles every cost figure derived from it. Staging files are written to a temp directory, never into the run.

**Alternatives considered:** (a) `google-cloud-bigquery` as a dependency: rejected on the measurement from ADR-34 — every added cloud library moves the demo image's transitive floor, and the `bq` CLI already ships with the gcloud SDK that deploys the service. (b) Streaming inserts from the pipeline: puts a cloud dependency on the critical path of a gate whose whole point is that it degrades honestly offline. (c) Cloud Logging sink with a BigQuery export: right answer on Agent Engine, where the spans land in Cloud Trace and Logging without this script at all; this is the offline path for runs that happen on a laptop. (d) `bq mk --force --view` to publish the views: it reports success and leaves an existing view's definition untouched, so a changed view keeps answering the old way — found while building this, replaced with `CREATE OR REPLACE VIEW`.

**Consequences:** The three runs release day produced are now queryable, and the two invalid ones are the most useful rows in the table: `v_never_completed` shows `claude API_ERROR ×2` for the run the credits died in and `gemini TIMEOUT ×1` for the run the laptop slept through. A failure is now something you query rather than something you argue about. The cost is a second place where the trace lives; the file stays the record of truth, the table is a copy, and nothing in the pipeline reads back from it.
