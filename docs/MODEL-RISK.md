...
## 4. Evaluation evidence (release [vX.Y], run [id], date [ ])

- Golden set: [N] documents ([n_pdf] PDF / [n_txt] text), [M] planted
  discrepancies across [k] finding types, [c] clean controls, mutation-
  testing style.
- Strict catch rate (field AND type): [x/M]. Field-level (diagnostic): [y/M].
- False-flag rate on clean fields: [f/F] (equal prominence by policy).
- Auto-clear correctness: [must be 100%] — nothing planted was auto-cleared.
- Cross-family extraction agreement: [ %].
- Parse tax (text vs parsed-PDF ablation): [summary; fields degraded listed
  in eval report].
