...
## 2026-09-14 — Freshness by clock, and other checks that were checking the wrong thing
- Situation: a second adversarial test pass on the observability work, briefed to assume the first pass had missed things. No P0 this time, but ten smaller findings, and three of them were the same mistake wearing different clothes.
- What broke / what we assumed: the provenance graph decided "has this changed" by comparing modification times, so a file stamped in the future stayed fresh forever and served a stale picture — and because every file in a copied run shares one timestamp, an equality comparison was carrying the entire decision in the state the demo actually ships. The demo's booking form decided "is this value acceptable" by trying a cast and falling back to the raw string, so `not-a-number` landed in a numeric field and the gate would have reported a type problem as a term problem. And a stub eval decided "did the reference lane run" from the requested flag rather than the effective one, so a run that never extracted the methodology scored 15/17 with nothing anywhere saying why.
- Lesson: each of those checks was cheap, plausible, and about a proxy rather than the thing. Hash what the artifact is made of; validate against the type the field already holds; record what actually happened, not what was asked for. The product's own rule — absence announces itself — applies to the tooling around it, not only to the extraction.
- Fix / rework: content fingerprint embedded in each graph; the booking form refuses a type mismatch and says which field; a stub run emits NOT_EVALUABLE for every index claim with the reason and the trace line now carries requested, loaded and why. Also: the answer key is no longer served next to the demo it grades, the mutable state directory is no longer committed, a full re-read is staged and swapped under the lock so a page can never pair new findings with an old summary, and the desk view is cached on a fingerprint of its inputs (31 ms → 2.4 ms, 10 → 277 requests a second).
- Post angle: "Three separate bugs, one shape: a cheap check that tested a proxy instead of the thing."
