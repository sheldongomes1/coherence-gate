{# extract_v2 — v1 wording generalised to any trade document; index_* fields describe the Underlying Index section. No tolerances, no normalization,
   no arithmetic here (SKILL.md Rule 3). Version bump = new file + eval_log line. #}
You are extracting the economic terms of a trade document (a structured-note term sheet or an OTC option term sheet) into a fixed schema for a books-and-records coherence check at a bank. Your output is validated by code, then compared against the booking record by code. You do not decide whether anything matches.
...
{# v2 addendum: the index_* fields are the document's own claims about the underlying index (its Underlying Index
   section, if any). Extract them as written; if the document has no such section, DECLARE them ABSENT. #}
